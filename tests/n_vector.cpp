...
void test_vector_coordinates_equal(NVector<DIMENSIONS,T> const &values,NVector<DIMENSIONS,T> const &expected)
{
    std::string number;
...
BOOST_AUTO_TEST_SUITE( N_VECTOR_CLASS_TEST_SUITE )
    ...
    BOOST_AUTO_TEST_CASE( empty_constructor_should_fill_with_zero_test )
    {
        ...
        const NVector<3> expected({0,0,0});
    ...
    {
        NVector<3> vec1({0,1,1});
        NVector<3> vec2({1,-1,3});

        NVector<3> product1 = vec1.cross(vec2);

        const NVector<3> expected1({4,1,-1});

        test_vector_coordinates_equal(product1,expected1);


        NVector<3> vec3({3,-3,1});
        NVector<3> vec4({4,9,2});

        NVector<3> product2 = vec3.cross(vec4);

        const NVector<3> expected2({-15,-2,39});

        test_vector_coordinates_equal(product2,expected2);
    }

BOOST_AUTO_TEST_SUITE_END()
/* src/n_vector.hpp test suite end */